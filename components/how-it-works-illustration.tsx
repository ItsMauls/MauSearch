...
const ILLUSTRATIONS: Record<string, React.ReactNode> = {
    ...
    <Box>
      ...
      {[0, 1, 2].map((i) => (
        <g key={i} className="animate-land" style={{ animationDelay: `${i * 200}ms` }}>
          <line x1="76" y1="17" x2="86" y2={30 + i * 18} className={line} strokeWidth="1.2" />
          <rect x="88" y={22 + i * 18} width="66" height="14" rx="4" className="fill-suggest-soft stroke-suggest" strokeWidth="1" />
          <circle cx="96" cy={29 + i * 18} r="2" className="fill-suggest" />
    ...
    <Box>
      {[46, 28, 16, 38].map((h, i) => (
        <rect
          ...
          height={h}
          rx="3"
          className="fill-brand-soft stroke-brand animate-land"
          strokeWidth="1"
  ...
  "4": (
    <Box>
      ...
      <rect x="26" y="38" width="24" height="16" rx="3" className="fill-rule-soft stroke-rule" strokeWidth="1.2" />
      <line x1="38" y1="54" x2="120" y2="66" className={line} strokeWidth="1.2" strokeDasharray="2 2" />
  ...
  "5": (
    <Box>
      ...
      <circle cx="118" cy="30" r="20" className="fill-ai-soft stroke-ai" strokeWidth="1.5" />
  ...
  "7": (
    <Box>
      <circle cx="26" cy="24" r="10" className="fill-accent/15 stroke-accent" strokeWidth="1.2" />
      ...
      <circle cx="80" cy="60" r="3" className="fill-accent animate-working" />
  ...
  "8": (
    <Box>
      ...
      <line x1="88" y1="30" x2="146" y2="30" className="stroke-danger" strokeWidth="1" />
...
export function StageIllustration({ n }: { n: string }) {
  ...
  if (!svg) return null;
  return (
    <div className="h-24 w-full shrink-0 overflow-hidden rounded-lg border border-line bg-canvas p-1.5 sm:h-20 sm:w-36">
      {svg}
    </div>
  );
}
