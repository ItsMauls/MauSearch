...
// Every route calling runStage caps at maxDuration=300s (Vercel's Fluid
// Compute budget - this project is new enough to default to it). Left
// unchecked, three sequential model calls (a truncation retry plus a
// schema-correction retry) can add up to more than that, and the platform
// kills the function outright with an opaque 504 - no error is recorded, no
// partial run state is saved. Stopping ourselves comfortably inside that
