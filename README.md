...
- **The default `AI_MODEL` is a free OpenRouter tier**, and measured under real load it took ~80 seconds for a single stage — comfortably past a typical serverless budget once four stages run in sequence. Route handlers declare `maxDuration = 300` to give it the most room a platform will realistically allow, and the UI says plainly when a free-tier model is configured so a long wait doesn't read as a hang. A paid tier such as `nvidia/nemotron-3-nano-30b-a3b` costs about $0.01 for a full run and responds in single-digit seconds — swap `AI_MODEL` (and fund the OpenRouter account) if reliability matters more than the free tier's price.
